...
-- Boolean expressions
--op3
step (B (Compare cop (AConst n1) (AConst n2)),s)
    = (B (BConst n), s)
    where n = bApply cop n1 n2
    
--op2
step (B (Compare cop (AConst n1) ae2),s)
    = (B (Compare cop (AConst n1) ae2'), s')
    where (A ae2', s')= step (A ae2, s)
    
--op1
step (B (Compare cop ae1 ae2),s)
    = (B (Compare cop  ae1' ae2), s')
    where (A ae1', s')= step (A ae1, s)
...
-- if
step (C (If (BConst True)  c1 c2),s)
    = (C c1,s)
step (C (If (BConst False) c1 c2),s)
    = (C c2,s)
step (C (If be c1 c2),s)
    = (C (If be' c1 c2),s')
...
-- do-whilst
step (C (DoWhilst c be),s) = (C (Seq c (If be(While be c)Skip)),s)
