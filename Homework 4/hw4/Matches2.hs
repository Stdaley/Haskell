...
rev_prop :: Reg -> ABCDStr -> Bool
rev_prop re (ABCD cs)
    = (matches re cs)==(matches (revExp re) (reverse cs))

---

revExp :: Reg -> Reg
revExp Empty = Empty
revExp Epsilon = Epsilon
revExp (Literal x) = (Literal x)
revExp (Or r1 r2) = Or (revExp r2) (revExp r1)
revExp (Then r1 r2) = Then (revExp r2) (revExp r1)
revExp (Star r) = Star (revExp r)
revExp (Opt r) = Opt (revExp r)
revExp (Plus r) = Plus (revExp r)
