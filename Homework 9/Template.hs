...
tryToLookUp x y zs
            | (length help) == 0 = y
            | otherwise = head help
            where help = [ b | (a,b) <- zs, x == a]


-- Pre: The given value is in the table
reverseLookUp :: Eq b => b -> [(a, b)] -> [a]
reverseLookUp x str = [a |(a,b) <- str, b==x]
  

occurs :: String -> Type -> Bool
occurs a (TFun b c) = occurs a b || occurs a c
occurs a (TVar b) = a == b
occurs a _ = False
...
inferType (Number a) _ = TInt
inferType (Boolean b) _ = TBool
inferType (Id a) b = lookUp a b
inferType (Prim a) b
          | a == ">" || a == "==" = TFun TInt (TFun TInt TBool)
          | a == "+" = TFun TInt (TFun TInt TInt)
          | a == "not" = TFun TBool TBool
          
inferType (Cond (Boolean a) b c) d
          | a = inferType b d
          | otherwise = inferType c d
inferType (App e1 e2) c = case (inferType e1 c, inferType e2 c) of
          (TFun u i, o) -> if u == o then i else TErr
          _ -> TErr
inferType _ _ = TErr
...
applySub lst (TVar x) = tryToLookUp x (TVar x) lst
applySub lst (TFun e1 e2) = TFun (applySub lst e1) (applySub lst e2)
applySub _ lst = lst
...
unifyPairs [] lst = Just lst

unifyPairs ( (TInt, TInt) : xs ) lst = helper
           where
           helper = unifyPairs xs lst

unifyPairs ( (TBool, TBool) : xs ) lst  = helper
           where
           helper = unifyPairs xs lst
           
unifyPairs ((( TVar t ),(TVar t') ): xs) lst
       | t == t' = helper
           where
           helper = unifyPairs xs lst
           
unifyPairs ( ( (TVar t), y) : xs) lst
           | occurs t y = Nothing
           | otherwise = helper
             where
                helper = unifyPairs (map y' xs) ( (t,y) : lst)
                y' (a,b) = ( (applySub e a), (applySub e b) )
                e = [ (t, y) ]
             
unifyPairs ( ( (TFun e1 e2), (TFun e1' e2') ): xs) lst = helper
           where
                helper = unifyPairs ( (e1, e1') : (e2, e2') : xs) lst
